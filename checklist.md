...
- [ ] Créer le fichier `.env` local (non versionné) et `.env.example`.
- [ ] Initialiser le client Supabase côté serveur (`src/lib/server/supabase.ts`).
- [ ] Initialiser le client Supabase côté client (ou partagé dans `src/lib/supabase.ts` avec `@supabase/ssr`).
- *Message de commit suggéré :* `feat: setup supabase client and environment variables`
...
- *Message de commit suggéré :* `feat: user authentication and profile management`
...
- *Message de commit suggéré :* `feat: music search, list, and detail views using deezer api`
...
- *Message de commit suggéré :* `feat: favorites management using svelte runes and supabase persistence`
...
- [ ] *Bonus* : Thème clair/sombre persistant.
- [ ] *Bonus* : Améliorations de l'accessibilité (a11y).
- *Message de commit suggéré :* `style: responsive design, client-side filters, and UI polishing`
...
- *Message de commit suggéré :* `ci: add github actions workflow and final readme`
